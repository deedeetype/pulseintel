...
BEGIN
  ...
  IF demo_user_id IS NULL THEN
    ...
    INSERT INTO users (id, clerk_id, email, name, plan)
    VALUES ('2db9243c-9c2b-4c3d-bd1e-48f80f39dfd7'::UUID, 'demo_user', 'demo@pulseintel.com', 'Demo User', 'pro')
  ...
  IF EXISTS (SELECT 1 FROM competitors WHERE industry = 'Video Games' AND scan_id IS NULL LIMIT 1) THEN
    ...
    UPDATE alerts SET scan_id = video_games_scan_id 
      WHERE scan_id IS NULL 
      AND (competitor_id IN (SELECT id FROM competitors WHERE scan_id = video_games_scan_id) OR competitor_id IS NULL);
    UPDATE insights SET scan_id = video_games_scan_id 
      WHERE scan_id IS NULL;
    UPDATE news_feed SET scan_id = video_games_scan_id 
      WHERE scan_id IS NULL;
  ...
  IF EXISTS (SELECT 1 FROM competitors WHERE industry = 'Financial Services' AND scan_id IS NULL LIMIT 1) THEN
    ...
    UPDATE alerts SET scan_id = financial_services_scan_id 
      WHERE scan_id IS NULL 
      AND created_at > NOW() - INTERVAL '3 hours';
    UPDATE insights SET scan_id = financial_services_scan_id 
      WHERE scan_id IS NULL 
      AND created_at > NOW() - INTERVAL '3 hours';
    UPDATE news_feed SET scan_id = financial_services_scan_id 
      WHERE scan_id IS NULL 
      AND created_at > NOW() - INTERVAL '3 hours';
